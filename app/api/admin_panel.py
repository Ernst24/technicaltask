...
@router.get(
    "/",
    summary="Просмотреть все правила доступа в системе! Кому что доступно! -[SUPPORT][MANAGER][ADMIN]",
)
...
@router.post("/", summary="Создать/изменить правило доступа [ADMIN]")
...
@router.post(
    "/ban/user/{ban_id}", summary="Бан пользователя - Мягкий (active=False)[ADMIN]"
)
async def ban_user(
    db: DBDep,
